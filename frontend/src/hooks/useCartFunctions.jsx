...
const useCartFunctions = () => {
  ...
  const addPlantToCart = async ({ _id, name }) => {
    ...
    if (!addLoading) {
      ...
      if (choosenPlant) {
        ...
        let id = _id
        await axios.post(`http://localhost:3000/api/cart/${id}`, {}, { headers: { Authorization: `Bearer ${user.token}` } })
          .then((response) => {
  ...
  const updatePlantInCart = async ({ id, change }) => {
    if (!updateLoading) {
      ...
      await axios.put(`http://localhost:3000/api/cart/${id}`, { change }, { headers: { Authorization: `Bearer ${user.token}` } })
        .then((response) => {
